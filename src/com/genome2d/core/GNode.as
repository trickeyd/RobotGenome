...
package com.genome2d.core
{
	...
	{                                
		...
		private var __eOnNodeAdded:Signal;
		/**
		 *         Signal that is dispatched when a child is added lower in this node tree.
		 */
		public function get onNodeAdded():Signal { 
			if (__eOnNodeAdded == null) __eOnNodeAdded = new Signal(GNode);
			return __eOnNodeAdded;
		}
		
		private var __eOnNodeRemoved:Signal;
		/**
		 *         Signal that is dispatched when a child is removed lower in this node tree.
		 */
		public function get onNodeRemoved():Signal { 
			if (__eOnNodeRemoved == null) __eOnNodeRemoved = new Signal(GNode);
			return __eOnNodeRemoved;
		}
		...
		public function dispose():void {
			...
			if(__eOnNodeAdded) {
				__eOnNodeAdded.removeAll();
				__eOnNodeAdded = null;
			}
			if(__eOnNodeRemoved) {
				__eOnNodeRemoved.removeAll();
				__eOnNodeRemoved = null;
			}
		...
		public function addChild(p_child:GNode, p_last:Boolean = true):void {
			...
			for(var node:GNode = p_child; node.cParent; node = node.cParent)
			{	
				if(node.cParent.__eOnNodeAdded)
					node.cParent.__eOnNodeAdded.dispatch(p_child);
			}
		...
		public function removeChild(p_child:GNode):void {
			...
			for(var node:GNode = p_child; node.cParent; node = node.cParent)
			{	
				if(node.cParent.__eOnNodeRemoved)
					node.cParent.__eOnNodeRemoved.dispatch(p_child);
			}
